class AuthsController < ApplicationController
  ...
  def show
    ...
    @auth =  Auth.where(:auth => params[:id]).first
    @weight_hash = Hash.new()
    Auth.where(:auth => params[:id]).each do |record|
      #puts record.topic_id
      @weight_hash[record.topic_id] = record.weight
    end
    respond_to do |format|
...
#  def getWeightstopic(auth)
#     wt_hash = Hash.new()
#     Auth.where(:auth => auth).each do |record|
#      puts record
#      wt_hash[record.topic_id] = record.weight
#     end
#     puts wt_hash.size()
#     return wt_hash
#  end
